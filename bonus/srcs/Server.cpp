...
const std::queue<Job>& Server::get_m_job_queue() const { return (this->m_job_queue); }
namespace {
	bool isWorkerFree(const Worker& worker)
	{
...
bool
Server::hasSendWork(Connection& connection, Worker *worker)
{
	...
	if (status != Connection::TO_SEND && status != Connection::ON_SEND)
		return (false);
	return (worker->fdIsset(fd, Worker::WRITE_COPY_SET));
}

bool
Server::runSend(Connection& connection, Worker *worker, bool& connect)
{
	Connection::Status status = connection.get_m_status();
	...
	if (ret)
	{
		...
		worker->fdClear(connection.get_m_client_fd(), Worker::WRITE_SET);
		...
		if (connection.get_m_response().get_m_status_code() / 100 != 2)
		{
			...
			closeConnection(connection.get_m_client_fd(), worker);
			connect = false;
		}
		else
			connection.clear();
...
bool
Server::hasExecuteWork(Connection& connection, Worker *worker)
{
	Connection::Status status = connection.get_m_status();
	...
	if (from_child_fd == -1 && to_child_fd == -1)
		return (false);
	if (from_child_fd != -1 && worker->fdIsset(from_child_fd, Worker::READ_COPY_SET))
		return (true);
	if (to_child_fd != -1 && worker->fdIsset(to_child_fd, Worker::WRITE_COPY_SET))
		return (true);
...
{
	void
	writeChunkedBodyToCGIScript(Worker* worker, Connection& connection, int connection_count)
	{
...
bool
Server::runExecute(Connection& connection, Worker *worker, bool& connect)
{
	int from_child_fd = connection.get_m_read_from_server_fd();
	int to_child_fd = connection.get_m_write_to_server_fd();
	int stat;
	...
	if (to_child_fd != -1 && worker->fdIsset(to_child_fd, Worker::WRITE_COPY_SET))
	{
		if (request.get_m_method() == Request::POST && request.get_m_transfer_type() == Request::CHUNKED)
			writeChunkedBodyToCGIScript(worker, connection, m_connections.size());
		else
			writeSavedBodyToCGIScript(worker, connection);
	}

	waitpid(connection.get_m_server_fd(), &stat, WNOHANG);
	if (WIFEXITED(stat) && !worker->fdIsset(to_child_fd, Worker::WRITE_SET)
	&& read_end == true)
	{
		if (from_child_fd != -1)
		{
			close(from_child_fd);
			worker->fdClear(from_child_fd, Worker::READ_SET);
		}
...
void
Server::closeConnection(int client_fd, Worker *worker)
{
	...
	worker->fdClear(client_fd, Worker::READ_SET);
	m_connections.erase(client_fd);
	// if (worker->get_max_fd() == client_fd)
	// 	worker->resetMaxFd();
}
...
bool Server::hasRequest(Connection& connection, Worker *worker)
{
	Connection::Status status = connection.get_m_status();
	...
	if (status != Connection::ON_WAIT && status != Connection::ON_RECV)
		return (false);
	if (!worker->fdIsset(fd, Worker::READ_COPY_SET))
		return (false);
	return (true);
}
...
void
Server::recvRequest(Connection& connection, const Request& const_request, Worker* worker)
{
	...
	if (phase == Request::READY && hasRequest(connection, worker) && (count = recvWithoutBody(connection, buf, sizeof(buf))) > 0)
...
bool
Server::runRecvAndSolve(Connection& connection, Worker* worker)
{
	try {
		recvRequest(connection, connection.get_m_request(), worker);
	...
	if (request.get_m_phase() == Request::COMPLETE)
	{
		...
		solveRequest(connection, connection.get_m_request(), worker);
		return (true);
	}
	return (false);
}
...
void
Server::exit() {
	std::vector<Worker>::iterator worker_it = m_workers.begin();
...
bool
Server::work(int client_fd, Worker *worker)
{
	(void)client_fd;
...
void
Server::solveRequest(Connection& connection, const Request& request, Worker* worker)
{
	...
	if (method == Request::TRACE)
		...
		return (executeCGI(connection, request, worker));
...
void
Server::executeCGI(Connection& connection, const Request& request, Worker *worker)
{
	pid_t pid;
	...
	if (method == Request::POST)
	{
		...
		worker->fdSet(connection.get_m_write_to_server_fd(), Worker::WRITE_SET);
	}
	else
		close(parent_write_fd[1]);
	connection.set_m_read_from_server_fd(child_write_fd[0]);
	worker->fdSet(connection.get_m_read_from_server_fd(), Worker::READ_SET);
...
Server::createResponse(Connection& connection, int status, headers_t headers, std::string body)
{
	...
	connection.set_m_status(Connection::TO_SEND);
	connection.get_m_worker()->fdSet(response.get_m_connection()->get_m_client_fd(), Worker::WRITE_SET);
}
